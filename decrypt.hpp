  ...
  /*
    for(unsigned int i = 0; i != encrypted_text.size(); i++){
    	...
    	if(	((encrypted_text[i] >= 'a') && (encrypted_text[i] <= 'z')) ){
            ...
            letter_count_EncrText[encrypted_text[i]-32]++;
            total_letters_amount_EncrText++;

    	}
        else if( ((encrypted_text[i] >= 'A') && (encrypted_text[i] <= 'Z')) ){

            letter_count_EncrText[i]++;
            total_letters_amount_EncrText++;
...
//string decrypt_ENG(const string encrypted_text, bool __enable___out_decrypted_text_1);
